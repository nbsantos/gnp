...
func (s Server) handle(clientAddr string, rrq ReadReq) {
	log.Printf("[%s] reqeusted file: %s", clientAddr, rrq.Filename)
...
NEXTPACKET:
	...
	RETRY:
		for i := s.Retries; i > 0; i-- {
			...
			// wait for the client's ACK packer
			...
			case errPkt.UnmarshalBinary(buf) == nil:
				log.Printf("[%s] received error: %v", clientAddr, err)
				return
			default:
				log.Printf("[%s] bad packed", clientAddr)
			}
		}
